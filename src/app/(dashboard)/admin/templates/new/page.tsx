...
export default function NewTemplatePage() {
  ...
  const [sections, setSections] = useState<ChecklistSection[]>([
    { id: defaultSectionId, title: "General Checks", description: "", items: [] }
  ]);
  ...
  useEffect(() => {
    ...
    setExpandedSections(new Set([defaultSectionId]));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error: insertError } = await supabase.from("checklist_templates").insert({
      name: form.name,
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#111827', margin: 0 }}>Create New Template</h1>
            <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Build a new checklist template for your machines</p>
          </div>
        </div>
          ...
          <div style={{ ...cardStyle, padding: '24px', height: 'fit-content', position: 'sticky', top: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="e.g. Daily Pre-Run Check" style={{ width: '100%', padding: '10px 12px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '14px' }} />
              </div>
              <div>
              ...
              <button type="submit" disabled={isSaving} style={{ width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px', background: '#0057A8', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '500', fontSize: '14px', cursor: 'pointer', opacity: isSaving ? 0.7 : 1 }}>
                <svg style={{ width: '16px', height: '16px' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
